...
def reconstruct_state(reconstructed_state_logits, state_feature_splits):
    ...
    for size in state_feature_splits:
        ...
        indices = (
            torch.multinomial(probs.view(-1, size), num_samples=1).view(B, H, W)
            + start_idx
        )
...
def train_vae(
    ...
    for i in range(num_epochs):
        ...
        loss = vae_loss(
            ...
            mu,
            logvar,
            model.embedd.state_feature_splits,
            model.args.beta,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-size", type=int, default=1024, help="Size of the dataset to generate"
    )
    parser.add_argument(
        "--latent-dim", type=int, default=8, help="Dimensionality of the latent space"
    )
    parser.add_argument(
        "--d-model",
        type=int,
    ...
    parser.add_argument(
        "--nhead", type=int, default=2, help="Number of attention heads"
    )
    parser.add_argument(
        "--num-encoder-layers", type=int, default=2, help="Number of encoder layers"
    )
    parser.add_argument(
        "--num-decoder-layers", type=int, default=2, help="Number of decoder layers"
    )
    parser.add_argument(
        "--dim-feedforward",
        type=int,
    ...
    parser.add_argument(
        "--epochs", type=int, default=3_000, help="Number of training epochs"
    )
    ...
    for epoch in range(epochs):
        for batch in train_loader:
            ...
            loss = vae_loss(
                reconstructed_state, x, mu, logvar, FEATURE_SPLITS, model.args.beta
            )
