...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        """x: (B, seq_len, d_model); returns x + positional encoding."""
...
class SpatialOpponentModel(nn.Module):
    def __init__(self, args: OMGArgs):
        ...
        # Embeds each (H, W, F) state into a d_model vector. Downsampling uses
        # AvgPool rather than strided convs (stride-2 convs fall off the
        # MKLDNN fast path and alias), and AvgPool rather than MaxPool to
        # preserve the sign of the motion channels (x - x_prev), where -1
        # marks a disappearance under fog of war.
        ...
        # Spatial head: d_model -> H*W heatmap of the opponent's location
    ...
    def forward(
        ...
        if cached_features:
            ...
            prev_flat = prev_states.reshape(B * T, H, W, F_dim)
            hist_feat = self.get_features(hist_flat, prev_flat).reshape(B, T, -1)
        ...
        # (B, d_model): summary token of the current state
