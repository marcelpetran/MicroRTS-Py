...
class ReplayBuffer:
    ...
    def sample_with_indices(self, batch_size: int):
        """Sample + return buffer slot indices (needed by the lambda-return
        tail walk, which locates a transition's episode successors in the
        circular buffer)."""
        idxs = random.sample(range(self.size), batch_size)
        return idxs, [self.buf[i] for i in idxs]
...
class ReservoirBufferExponentialAveraging:
    ...
    def sample(self, batch_size: int):
        ...
        if self.decay_tau is None:
            ...
            # true exponential
            weights = torch.exp((self.arrival[:n] - self.n_seen) / self.decay_tau)
