...
from omexplore.models.networks import QNetClassic, SLnet
...
class FSPAgentClassic:
    ...
    Refactored to use the POSG env API:
      - Belief-augmented states for Q-learning (QNetClassic expects belief channels).
      - SL network continues to use raw observations (SLnet has no belief channels).
    """
    ...
    @torch.no_grad()
    def select_rl_action(self, s_aug: np.ndarray, eval=False) -> tuple[int, float]:
        """Best Response action using classic Q-learning (belief-augmented state)."""
        self.q.eval()
        x = torch.from_numpy(s_aug).float().unsqueeze(0).to(self.device)
    ...
    def select_action(
        self, s_t: np.ndarray, s_aug: np.ndarray = None, eval=False
    ) -> tuple[int, float]:
        ...
        return self.select_rl_action(s_aug if s_aug is not None else s_t, eval=eval)

    # ------------- RL Update Logic -------------

    def compute_targets(self, batch: List[Dict]) -> tuple[torch.Tensor, torch.Tensor]:
        """DDQN target computation (belief-augmented states stored in replay)."""
        s = (
    ...
    def update_sl(self):
        """Updates the SL Average Strategy network (raw states)."""
    ...
    def run_fsp_episode(
        ...
        share this agent's networks; each maintains its own belief tracker.
        RL buffer stores belief-augmented states; SL buffer stores raw states.
        """
        obs = self.env.reset()
        ...
        for step in range(max_steps):
            # Player 0: compute RL (BR) and SL (Average) actions
            s_aug = tracker_0.augment(obs[0])
            rl_a, rl_entropy = self.select_rl_action(s_aug)
            ...
            if (
                ...
                opp_rl_a, _ = opponent_agent.select_rl_action(opp_s_aug)
            ...
            # RL buffer: belief-augmented states (QNetClassic expects belief channels)
            transition = {
                "state": s_aug.copy(),
                "action": a,
    ...
    def run_test_episode(
        ...
        for step in range(max_steps):
            ...
            if use_sl:
                ...
                a, step_entropy = self.select_rl_action(s_aug, eval=True)
            ...
            if (
                ...
                a_opponent, _ = opponent_agent.select_action(
                    opp_s_aug, eval=eval if use_sl else True
                )
        ...
        return {
            ...
            "steps": (step + 1) if max_steps > 0 else 0,
            "opp_return": opp_ret,
            "avg_rl_entropy": ep_entropy / (step + 1)
            if max_steps > 0 and not use_sl
            else 0.0,
            "avg_sl_entropy": ep_entropy / (step + 1)
