...
parser.add_argument(
    ...
    type=int,
    default=1,
    choices=[i for i in range(1, len(map_layouts) + 1)],
    help="Map layout to use",
)
parser.add_argument(
...
parser.add_argument(
    "--qnet_dim", type=int, default=256, help="Hidden dimension for Q-network"
)
parser.add_argument(
    "--cnn_hidden", type=int, default=64, help="Hidden dimension for CNN in Q-network"
)
parser.add_argument(
    "--d_model", type=int, default=64, help="Transformer model dimension"
)
parser.add_argument("--nhead", type=int, default=4, help="Number of attention heads")
parser.add_argument(
    "--num_encoder_layers", type=int, default=1, help="Number of encoder layers"
)
parser.add_argument(
    "--dim_feedforward", type=int, default=256, help="Dimension of feedforward network"
)
...
parser.add_argument(
    "--tau_end", type=float, default=0.1, help="Ending temperature value for Boltzmann"
)
parser.add_argument(
    "--tau_decay_steps",
    type=int,
...
parser.add_argument("--train_every", type=int, default=2, help="Train every N steps")
parser.add_argument(
...
parser.add_argument(
    "--folder_id", type=int, default=0, help="Folder ID for saving models and diagrams"
)
parser.add_argument(
    "--true_intent",
    action="store_true",
...
for epoch in range(num_epochs):
    ...
    pbar = tqdm(
        ...
        desc=f"Phase 1 Epoch {epoch + 1:02d}/{num_epochs} [Train]",
        leave=False,
    )
    for ep in pbar:
    ...
    print(
        f"Phase 1 | Epoch {epoch + 1:02d} | Train Ret: {avg_train_ret:>4.1f} | Eval Ret: {avg_eval_ret:>4.2f} | Eval Opp: {avg_eval_opp:>4.2f} | Entropy: {avg_entropy:.4f}"
    )
...
for epoch in range(num_epochs):
    ...
    pbar = tqdm(
        range(args_parsed.episodes_per_epoch),
        desc=f"Phase 2 Epoch {epoch + 1:02d}/{num_epochs} [Train]",
        leave=False,
    )
    for ep in pbar:
    ...
    print(
        f"Phase 2 | Epoch {epoch + 1:02d} | Train Ret: {avg_train_ret:>4.1f} | Eval Ret: {avg_eval_ret:>4.2f} | Eval Opp: {avg_eval_opp:>4.2f} | Entropy: {avg_entropy:.4f} | Q Loss: {avg_q_loss:.4f} | Model Loss: {avg_model_loss:.4f}"
    )
...
plt.plot(
    episode_list,
    phase1_eval_opp_returns,
    label="Opponent (vs Classic)",
    color="red",
    linestyle="--",
)
plt.plot(
    episode_list, phase2_eval_opp_returns, label="Opponent (vs OM)", color="orange"
)
plt.xlabel("Training Episodes")
plt.ylabel("Average Eval Return")
...
plt.subplot(1, 3, 3)
plt.plot(
    episode_list, phase1_eval_steps, label="Classic Agent", color="blue", linestyle="--"
)
