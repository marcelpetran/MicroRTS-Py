...
from omexplore.agents.slq_agent import FSPAgentOM
from omexplore.agents.slq_agent_classic import FSPAgentClassic
...
parser.add_argument(
    "--episodes", type=int, default=12_000, help="Total training episodes per phase"
)
parser.add_argument(
    "--episodes_per_epoch", type=int, default=500, help="Episodes per epoch"
)
parser.add_argument(
    "--pretrain_epochs", type=int, default=15, help="OM Pretraining epochs"
)
...
parser.add_argument(
    "--num_encoder_layers", type=int, default=1, help="Transformer layers"
)
parser.add_argument(
    "--dim_feedforward", type=int, default=256, help="Transformer feedforward dim"
)
...
parser.add_argument(
    "--tau_decay_steps", type=int, default=600_000, help="Boltzmann decay steps"
)
parser.add_argument("--train_every", type=int, default=2, help="Train frequency")
parser.add_argument(
    "--replay_capacity", type=int, default=150_000, help="Replay buffer capacity"
)
...
for epoch in range(num_epochs):
    ...
    eta = max(
        0.1,
        1.0
        - (epoch * args_parsed.episodes_per_epoch)
        / (num_epochs * args_parsed.episodes_per_epoch),
    )
    (
        epoch_returns,
        epoch_opp_returns,
    ...
    torch.save(
        agent_classic.q.state_dict(),
        f"./models/{args.folder_id}/classic_qnet_ep{epoch + 1}.pth",
    )
    torch.save(
        agent_classic.sl.state_dict(),
        f"./models/{args.folder_id}/classic_slnet_ep{epoch + 1}.pth",
    )

    print(
        f"Phase 1 | Epoch {epoch + 1:02d} | Classic Ret: {avg_ret:>4.1f} | Opp Ret: {avg_opp:>4.1f} | RL Entropy: {avg_rl_ent:.4f} | SL Entropy: {avg_sl_ent:.4f}| SL Buffer: {len(agent_classic.sl_replay)}"
    )
...
for epoch in range(num_epochs):
    ...
    torch.save(
        agent_om.q.state_dict(), f"./models/{args.folder_id}/om_qnet_ep{epoch + 1}.pth"
    )
    torch.save(
        agent_om.sl.state_dict(),
        f"./models/{args.folder_id}/om_slnet_ep{epoch + 1}.pth",
    )
    torch.save(
        agent_om.model.inference_model.state_dict(),
...
def evaluate_matchup(agent0, agent1, env, args, use_sl=False):
    """
    Safely executes a matchup, handling belief tracking and the rolling GPU
    history correctly for whichever agent contains the Opponent Modeling
    architecture (state-only history, no opponent actions).
    """
    ...
    tracker_0 = BeliefTracker(
        args.H, args.W, map_layout=env.map_layout, horizon=args.max_steps
    )
    ...
    agent0.reset()
    agent1.reset()
    tracker_0.reset(use_map_prior=args.belief_map_prior)
    tracker_1.reset(use_map_prior=args.belief_map_prior)
    tracker_0.update(obs[0])
    tracker_1.update(obs[1])

    # OM history containers (state-only: features + mask + prev_obs).
    # Always allocated — only used when the corresponding agent is an FSPAgentOM.
    hist_len = args.max_history_length
    rf0 = torch.zeros((1, hist_len, args.d_model), device=args.device)
    rm0 = torch.zeros((1, hist_len), dtype=torch.bool, device=args.device)
    prev0 = torch.zeros((1, *obs[0].shape), device=args.device)
    c_seq0 = 0
    rf1 = torch.zeros((1, hist_len, args.d_model), device=args.device)
    rm1 = torch.zeros((1, hist_len), dtype=torch.bool, device=args.device)
    prev1 = torch.zeros((1, *obs[1].shape), device=args.device)
    c_seq1 = 0

    ret0, ret1 = 0, 0

    for step in range(args.max_steps):
        s_aug0 = tracker_0.augment(obs[0])
        s_aug1 = tracker_1.augment(obs[1])

        # Agent 0 action
        if agent0_is_om:
            h0 = {"state_features": rf0, "mask": rm0, "prev_obs": prev0}
            if use_sl:
                a0, _ = agent0.select_sl_action(obs[0], eval=True)
            else:
                a0, _, _ = agent0.select_rl_action(obs[0], s_aug0, h0, eval=True)
        else:
            if use_sl and hasattr(agent0, "select_sl_action"):
                a0, _ = agent0.select_sl_action(obs[0], eval=True)
            else:
                a0, _ = (
                    agent0.select_rl_action(s_aug0, eval=True)
                    if hasattr(agent0, "select_rl_action")
                    else agent0.select_action(obs[0])[:2]
                )

        # Agent 1 action
        if agent1_is_om:
            h1 = {"state_features": rf1, "mask": rm1, "prev_obs": prev1}
            if use_sl:
                a1, _ = agent1.select_sl_action(obs[1], eval=True)
            else:
                a1, _, _ = agent1.select_rl_action(obs[1], s_aug1, h1, eval=True)
        else:
            if use_sl and hasattr(agent1, "select_sl_action"):
                a1, _ = agent1.select_sl_action(obs[1], eval=True)
            else:
                a1, _ = (
                    agent1.select_rl_action(s_aug1, eval=True)
                    if hasattr(agent1, "select_rl_action")
                    else agent1.select_action(obs[1])[:2]
                )
        ...
        # Update OM histories (state-only, motion-encoded features)
        if agent0_is_om:
            s_tens = torch.from_numpy(obs[0]).float().unsqueeze(0).to(agent0.device)
            with torch.no_grad():
                nf = agent0.model.inference_model.get_features(s_tens, prev0)
        ...
        if agent1_is_om:
            ...
            with torch.no_grad():
                nf = agent1.model.inference_model.get_features(s_tens, prev1)
...
for opp_name, heuristic_opp in heuristics.items():
    ...
    for _ in range(total_eval_episodes):
        ...
        o_ret, _ = evaluate_matchup(agent_om, heuristic_opp, env, args, use_sl=True)
...
for _ in range(total_eval_episodes):
    ...
    c_ret, o_ret = evaluate_matchup(agent_classic, agent_om, env, args, use_sl=True)
    classic_cross_returns.append(c_ret)
    om_cross_returns.append(o_ret)

    # Matchup B: OM (0) vs Classic (1)
    o_ret, c_ret = evaluate_matchup(agent_om, agent_classic, env, args, use_sl=True)
    classic_cross_returns.append(c_ret)
    om_cross_returns.append(o_ret)
...
plt.subplot(2, 2, 1)
plt.plot(
    episode_list, phase1_returns, label="Classic Agent", color="blue", linestyle="--"
)
...
plt.plot(
    episode_list,
    phase1_rl_entropies,
    label="Classic Agent RL Entropy",
    color="blue",
    linestyle="--",
)
plt.plot(
    episode_list,
    phase1_sl_entropies,
    label="Classic Agent SL Entropy",
    color="red",
    linestyle="--",
)
plt.plot(episode_list, phase2_rl_entropies, label="OM Agent RL Entropy", color="green")
plt.plot(episode_list, phase2_sl_entropies, label="OM Agent SL Entropy", color="orange")
plt.xlabel("Episodes")
